...
func TestMongodbRedoLogger(t *testing.T) {
	vdbEnv.init(t, nil)
	...
	commitToRedologAndRestart := func(newVal string, version *version.Height) {
		...
		db, err := vdbEnv.DBProvider.GetDBHandle("testmongodbredologger", nil)
		require.NoError(t, err)
		vdb := db.(*VersionedDB)
		require.NoError(t,
	...
	db, _ = vdbEnv.DBProvider.GetDBHandle("testmongodbredologger", nil)
	vdb = db.(*VersionedDB)

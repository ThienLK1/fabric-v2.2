...
const (
	...
	// Due to MongoDB's length restriction on db names, channel names and namepsaces may be truncated in db names.
	// The metadata is used for dropping channel-specific databases and snapshot support.
	channelMetadataDocID = "channel_metadata"

	// fabricInternalDBName is used to create a db in mongo that would be used for internal data such as the version of the data format
...
// default dbname mongo init
var mongoInitDBNames = [3]string{"admin","config","local"}
...
func NewVersionedDBProvider(config *ledger.MongoDBConfig, metricsProvider metrics.Provider, sysNamespaces []string) (*VersionedDBProvider, error) {
	logger.Debugf("constructing MongoDB VersionedDBProvider config %s",config)
	mongoInstance, err := createMongoInstance(config, metricsProvider)
	if err != nil {
		return nil, err
	}
	if err := checkExpectedDataformatVersion(mongoInstance); err != nil {
		return nil, err
	}
...
func checkExpectedDataformatVersion(mongoInstance *mongoInstance) error {
	databasesToIgnore := []string{fabricInternalDBName}
	for _, s := range mongoInitDBNames{
		databasesToIgnore = append(databasesToIgnore,s)
	}
	isEmpty, err := mongoInstance.isEmpty(databasesToIgnore)
	if err != nil {
		return err
	}
	if isEmpty {
		logger.Debugf("mongo instance is empty. Setting dataformat version to %s", dataformat.CurrentFormat)
		return writeDataFormatVersion(mongoInstance, dataformat.CurrentFormat)
	}
	dataformatVersion, err := readDataformatVersion(mongoInstance)
	if err != nil {
		return err
	}
	logger.Debugf("read Data format version to %s", dataformatVersion)
	// logger.Debugf("read Data format version to %s", dataformatVersion.Version)
	logger.Debugf("dataformat.CurrentFormat %s", dataformat.CurrentFormat)

	if dataformatVersion != dataformat.CurrentFormat {
		return &dataformat.ErrFormatMismatch{
			DBInfo:         "MongoDB for state database",
...
func readDataformatVersion(mongoInstance *mongoInstance) (string, error) {
	db, err := createMongoCollection(mongoInstance, fabricInternalDBName)
	if err != nil {
		return "", err
	}
	logger.Debugf("readDataformatVersion db %s", db)
...
func writeDataFormatVersion(mongoInstance *mongoInstance, dataformatVersion string) error {
	logger.Debugf("writeDataFormatVersion dataformatVersion %s", dataformatVersion)
	db, err := createMongoCollection(mongoInstance, fabricInternalDBName)
	if err != nil {
		logger.Errorf("writeDataFormatVersion err %s", err.Error())
		return err
	}
	doc, err := encodeDataformatInfo(dataformatVersion)
	if err != nil {
		logger.Errorf("encodeDataformatInfo err %s", err.Error())
		return err
	}
	result, err := db.saveDoc(dataformatVersionDocID, "", doc)
	
	logger.Debugf("saveDoc result = %s", result)
	if err != nil {
		logger.Errorf("save doc dataformatVersionDocID err %s", err.Error())
		return err
	}
	return err
}
...
func (provider *VersionedDBProvider) Close() {
	// No close needed on Mongo
	provider.redoLoggerProvider.close()
}

// HealthCheck checks to see if the mongo instance of the peer is healthy
func (provider *VersionedDBProvider) HealthCheck(ctx context.Context) error {
	return provider.mongoInstance.healthCheck(ctx)
}

// VersionedDB implements VersionedDB interface
type VersionedDB struct {
	mongoInstance      *mongoInstance
	metadataDB         *mongoDatabase            // A database per channel to store metadata such as savepoint.
	chainName          string                    // The name of the chain/channel.
	namespaceDBs       map[string]*mongoDatabase // One database per namespace.
...
func newVersionedDB(mongoInstance *mongoInstance, redoLogger *redoLogger, colName string, cache *cache, nsProvider statedb.NamespaceProvider) (*VersionedDB, error) {
	// createMongoCollection creates a MongoDB database object, as well as the underlying database if it does not exist
	chainName := colName
	colName = constructMetadataCOLName(colName)

	metadataDB, err := createMongoCollection(mongoInstance, colName)
	if err != nil {
		return nil, err
	}
	namespaceDBMap := make(map[string]*mongoDatabase)
	vdb := &VersionedDB{
		mongoInstance:      mongoInstance,
...
func (vdb *VersionedDB) getNamespaceDBHandle(namespace string) (*mongoDatabase, error) {
	vdb.mux.RLock()
	...
	namespaceDBName := constructNamespaceDBName(vdb.chainName, namespace, vdb.metadataDB.DatabaseName)
	vdb.mux.Lock()
	...
	if db == nil {
		...
		db, err = createMongoCollection(vdb.mongoInstance, namespaceDBName)
		if err != nil {
			return nil, err
		}
...
// A bulk retrieve from mongodb is used to populate the cache.
// committedVersions cache will be used for state validation of readsets
// revisionNumbers cache will be used during commit phase for mongodb bulk updates
	...
	for ns, nsMetadata := range nsMetadataMap {
		for _, keyMetadata := range nsMetadata {
			...
			if len(keyMetadata.Version) != 0 {
				...
				rev := string(keyMetadata.Rev)
				committedDataCache.setVerAndRev(ns, keyMetadata.ID, version, rev)
			}
		}
	}
...
func (vdb *VersionedDB) GetStateRangeScanIteratorWithPagination(namespace string, startKey string, endKey string, pageSize int32) (statedb.QueryResultsIterator, error) {
	...
	queryLimit := vdb.mongoInstance.queryLimit()
	db, err := vdb.getNamespaceDBHandle(namespace)
	if err != nil {
		return nil, err
	}
	return newQueryScanner(namespace, db, "", queryLimit, pageSize, "", startKey, endKey)
}
...
func (scanner *queryScanner) getNextStateRangeScanResults() error {
	...
	queryResult, nextStartKey, err := rangeScanFilterMongoInternalDocs(scanner.db,
...
func rangeScanFilterMongoInternalDocs(db *mongoDatabase,
	...
	var err error
	for i := 0; isMongoInternalKey(finalNextStartKey); i++ {
...
func isMongoInternalKey(key string) bool {
...
func (vdb *VersionedDB) ExecuteQueryWithPagination(namespace, query, bookmark string, pageSize int32) (statedb.QueryResultsIterator, error) {
	...
	queryLimit := vdb.mongoInstance.queryLimit()
	queryString, err := applyAdditionalQueryOptions(query, queryLimit, bookmark)
	if err != nil {
	...
	return newQueryScanner(namespace, db, queryString, queryLimit, pageSize, bookmark, "", "")
}
...
func (scanner *queryScanner) executeQueryWithBookmark() error {
	queryLimit := scanner.queryDefinition.queryLimit
	if scanner.paginationInfo.requestedLimit > 0 {
		if scanner.paginationInfo.requestedLimit-scanner.resultsInfo.totalRecordsReturned < scanner.queryDefinition.queryLimit {
...
func (vdb *VersionedDB) ApplyUpdates(updates *statedb.UpdateBatch, height *version.Height) error {
	...
	logger.Debugf("ApplyUpdates call applyUpdates: %s\n", vdb)
...
func (vdb *VersionedDB) applyUpdates(updates *statedb.UpdateBatch, height *version.Height) error {
	...
	logger.Debugf("stage 2 -- executeCommitter executes each committer to push the changes to the DB %s", vdb)
...
func (vdb *VersionedDB) postCommitProcessing(committers []*committer, namespaces []string, height *version.Height) error {
	...
	// 		return err
	// 	}
	// 	if db.mongoInstance.conf.WarmIndexesAfterNBlocks > 0 {
	// 		if db.indexWarmCounter >= db.mongoInstance.conf.WarmIndexesAfterNBlocks {
...
func (vdb *VersionedDB) Open() error {
	// no need to open db since a shared mongo instance is used
	return nil
}

// Close implements method in VersionedDB interface
func (vdb *VersionedDB) Close() {
	// no need to close db since a shared mongo instance is used
}
...
func (vdb *VersionedDB) recordSavepoint(height *version.Height) error {
	logger.Debugf("recordSavepoint")
	defer logger.Debugf("exit recordSavepoint")
	...
	if err != nil {
		logger.Errorf("Failed to encodeSavepoint to DB %s", err.Error())
		return err
	}
...
func (vdb *VersionedDB) initChannelMetadata(isNewDB bool, namespaceProvider statedb.NamespaceProvider) error {
	...
	dbNames, err := vdb.mongoInstance.retrieveApplicationDBNames()
	if err != nil {
		return err
	}
	...
	for _, ns := range namespaces {
		dbName := constructNamespaceDBName(vdb.chainName, ns, vdb.metadataDB.DatabaseName)
...
func (vdb *VersionedDB) readChannelMetadata() (*channelMetadata, error) {
	var err error
	mongoDoc, _, err := vdb.metadataDB.readDoc(channelMetadataDocID)
	if err != nil {
	...
	return decodeChannelMetadata(mongoDoc)
}

// writeChannelMetadata saves channel metadata to metadataDB
func (vdb *VersionedDB) writeChannelMetadata() error {
	mongoDoc, err := encodeChannelMetadata(vdb.channelMetadata)
	if err != nil {
		return err
	}
	_, err = vdb.metadataDB.saveDoc(channelMetadataDocID, "", mongoDoc)
	return err
}
...
func (vdb *VersionedDB) GetFullScanIterator(skipNamespace func(string) bool) (statedb.FullScanIterator, byte, error) {
	...
	return newDBsScanner(dbsToScan, vdb.mongoInstance.queryLimit(), toSkipKeysFromEmptyNs)
}
...
func newQueryScanner(namespace string, db *mongoDatabase, query string, queryLimit,
	limit int32, bookmark, startKey, endKey string) (*queryScanner, error) {
	scanner := &queryScanner{namespace, db, &queryDefinition{startKey, endKey, query, queryLimit}, &paginationInfo{-1, limit, bookmark}, &resultsInfo{0, nil}, false}
	var err error
...
func (scanner *queryScanner) next() (*mongoDoc, error) {
	...
	return &mongoDoc{
		...
		binaryDatas: result.binaryDatas,
	}, nil
}
...
func (s *dbsScanner) Next() (*statedb.CompositeKey, []byte, error) {
	...
	for {
		...
		if s.currentNamespace == "" {
			key, err := mongoDoc.key()
			if err != nil {
				return nil, nil, errors.WithMessagef(
					err,
					"failed to retrieve key from the mongodoc present in the empty namespace",
				)
			}
		...
		fields, err := validateAndRetrieveFields(mongoDoc)
		if err != nil {
			return nil, nil, errors.WithMessagef(
				err,
				"failed to validate and retrieve fields from mongo doc with id %s",
				fields.id,
			)
		}

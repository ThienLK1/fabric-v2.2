...
// testVDBEnv provides a mongo db backed versioned db for testing
type testVDBEnv struct {
	t             *testing.T
	DBProvider    statedb.VersionedDBProvider
	config        *ledger.MongoDBConfig
	cache         *cache
	sysNamespaces []string
	mongoDBEnv    *testMongoDBEnv
}

func (env *testVDBEnv) init(t *testing.T, sysNamespaces []string) {
	t.Logf("Initializing TestVDBEnv")

	if env.mongoDBEnv == nil {
		mongoDBEnv := &testMongoDBEnv{}
		mongoDBEnv.startMongoDB(t)
		env.mongoDBEnv = mongoDBEnv
	}
	...
	config := &ledger.MongoDBConfig{
		Address:             env.mongoDBEnv.mongoAddress,
		Username:            "admin",
		Password:            "adminpw",
...
func (env *testVDBEnv) cleanup() {
	...
	env.mongoDBEnv.cleanup(env.config)
	os.RemoveAll(env.config.RedoLogPath)
}

// testVDBEnv provides a mongo db for testing
type testMongoDBEnv struct {
	t              *testing.T
	mongoAddress   string
	cleanupMongoDB func()
}

// startMongoDB starts external mongoDB resources for testMongoDBEnv.
func (env *testMongoDBEnv) startMongoDB(t *testing.T) {
	if env.mongoAddress != "" {
		return
	}
	env.t = t
	env.mongoAddress, env.cleanupMongoDB = StartMongoDB(t, nil)
}

// stopMongoDB stops external mongoDB resources.
func (env *testMongoDBEnv) stopMongoDB() {
	if env.mongoAddress != "" {
		env.cleanupMongoDB()
	}
}

func (env *testMongoDBEnv) cleanup(config *ledger.MongoDBConfig) {
	err := DropApplicationDBs(config)
	require.NoError(env.t, err)
}

// we create two MongoDB instances/containers---one is used to test the
// functionality of the versionedDB and another for testing the MongoDB
// util functions.
var vdbEnv = &testVDBEnv{}
var mongoDBEnv = &testMongoDBEnv{}

func TestMain(m *testing.M) {
	flogging.ActivateSpec("statemongodb=debug")

	rc := m.Run()
	if vdbEnv.mongoDBEnv != nil {
		vdbEnv.mongoDBEnv.stopMongoDB()
	}
	mongoDBEnv.stopMongoDB()
	os.Exit(rc)
}
...
// The following tests are unique to mongodb, they are not used in leveldb
//  query test
...
func TestUtilityFunctions(t *testing.T) {
	...
	// BytesKeySupported should be false for MongoDB
	...
	require.EqualError(t, db.ValidateKeyValue("", []byte("validValue")),
		"invalid key. Empty string is not supported as a key by mongodb")
...
func TestHandleChaincodeDeploy(t *testing.T) {
	...
	indexData := map[string][]byte{
		"META-INF/statedb/mongodb/indexes/indexColorSortName.json":                                               []byte(`{"index":{"fields":[{"color":"desc"}]},"ddoc":"indexColorSortName","name":"indexColorSortName","type":"json"}`),
		"META-INF/statedb/mongodb/indexes/indexSizeSortName.json":                                                []byte(`{"index":{"fields":[{"size":"desc"}]},"ddoc":"indexSizeSortName","name":"indexSizeSortName","type":"json"}`),
		"META-INF/statedb/mongodb/collections/collectionMarbles/indexes/indexCollMarbles.json":                   []byte(`{"index":{"fields":["docType","owner"]},"ddoc":"indexCollectionMarbles", "name":"indexCollectionMarbles","type":"json"}`),
		"META-INF/statedb/mongodb/collections/collectionMarblesPrivateDetails/indexes/indexCollPrivDetails.json": []byte(`{"index":{"fields":["docType","price"]},"ddoc":"indexPrivateDetails", "name":"indexPrivateDetails","type":"json"}`),
	}
	...
	if !ok {
		t.Fatalf("Mongodb state impl is expected to implement interface `statedb.IndexCapable`")
	}
...
func TestIndexDeploymentWithOrderAndBadSyntax(t *testing.T) {
	...
	indexData := map[string][]byte{
		"META-INF/statedb/mongodb/indexes/indexColorSortName.json":                             []byte(`{"index":{"fields":[{"color":"desc"}]},"ddoc":"indexSizeSortName","name":"indexSizeSortName","type":"json"}`),
		"META-INF/statedb/mongodb/indexes/indexSizeSortName.json":                              []byte(`{"index":{"fields":[{"size":"desc"}]},"ddoc":"indexSizeSortName","name":"indexSizeSortName","type":"json"}`),
		"META-INF/statedb/mongodb/indexes/badSyntax.json":                                      []byte(badSyntaxFileContent),
		"META-INF/statedb/mongodb/collections/collectionMarbles/indexes/indexCollMarbles.json": []byte(`{"index":{"fields":["docType","owner"]},"ddoc":"indexCollectionMarbles", "name":"indexCollectionMarbles","type":"json"}`),
	}
...
func TestIsBulkOptimizable(t *testing.T) {
	...
	if !ok {
		t.Fatal("state mongo db is expected to implement interface statedb.BulkOptimizable")
	}
}
...
func TestRangeScanWithMongoInternalDocsPresent(t *testing.T) {
	vdbEnv.init(t, nil)
	defer vdbEnv.cleanup()
	db, err := vdbEnv.DBProvider.GetDBHandle("testrangescanfiltermongointernaldocs", nil)
	require.NoError(t, err)
	mongoDatabse, err := db.(*VersionedDB).getNamespaceDBHandle("ns")
	require.NoError(t, err)
	db.Open()
	defer db.Close()
	_, err = mongoDatabse.createIndex(`{
	...
	s, err = newQueryScanner("ns", mongoDatabse, "", 2, 2, "", "", "")
	require.NoError(t, err)
	...
	s, err = newQueryScanner("ns", mongoDatabse, "", 2, 2, "", "_", "")
	require.NoError(t, err)
	assertQueryResults(t, s.resultsInfo.results, []string{"key-1", "key-2"})
	require.Equal(t, "key-3", s.queryDefinition.startKey)
}
...
func testFormatCheck(t *testing.T, dataFormat string, dataExists bool, expectedErr *dataformat.ErrFormatMismatch, expectedFormat string, vdbEnv *testVDBEnv) {
	...
	config := &ledger.MongoDBConfig{
		Address:             vdbEnv.mongoDBEnv.mongoAddress,
		Username:            "admin",
		Password:            "adminpw",
		MaxRetries:          3,
		MaxRetriesOnStartup: 20,
	...
	if dataFormat == "" {
		response, err := dropDB(dbProvider.mongoInstance, fabricInternalDBName)
		require.NoError(t, err)
		require.True(t, response.Ok)
	} else {
		require.NoError(t, writeDataFormatVersion(dbProvider.mongoInstance, dataFormat))
	}
	dbProvider.Close()
	defer func() {
	...
	format, err := readDataformatVersion(dbProvider.mongoInstance)
	require.NoError(t, err)
...
func testExistInCache(t *testing.T, db *mongoDatabase, cache *cache, chainID, ns, key string, expectedVV *statedb.VersionedValue) {
	cacheValue, err := cache.getState(chainID, ns, key)
	require.NoError(t, err)
	vv, err := constructVersionedValue(cacheValue)
	require.NoError(t, err)
...
func TestChannelMetadata_NegativeTests(t *testing.T) {
	...
	origMongoAddress := vdbEnv.config.Address
	...
	expectedErrMsg := fmt.Sprintf("http error calling mongodb: Get \"http://%s/testchannelmetadata-errorpropagation_\": dial tcp %s: connect: connection refused",
		vdbEnv.config.Address, vdbEnv.config.Address)
	_, err := vdbEnv.DBProvider.GetDBHandle(channelName, nil)
	require.EqualError(t, err, expectedErrMsg)
	vdbEnv.config.Address = origMongoAddress
	...
	vdb := db.(*VersionedDB)
	vdbEnv.config.Address = "127.0.0.1:1"
	expectedErrMsg = fmt.Sprintf("http error calling mongodb: Put \"http://%s/testchannelmetadata-errorpropagation_/channel_metadata\": dial tcp %s: connect: connection refused",
		vdbEnv.config.Address, vdbEnv.config.Address)
	_, err = vdb.getNamespaceDBHandle("testnamepsace1")
	require.EqualError(t, err, expectedErrMsg)
	vdb.mongoInstance.conf.Address = origMongoAddress

	// call createMongoCollection to simulate peer crashes after metadataDB is created but before channelMetadata is updated
	...
	mongoInstance, err := createMongoInstance(vdbEnv.config, &disabled.Provider{})
	require.NoError(t, err)
	metadatadbName := constructMetadataDBName(channelName)
	metadataDB, err := createMongoCollection(mongoInstance, metadatadbName)
	require.NoError(t, err)
	vdb = &VersionedDB{
...
func TestInitChannelMetadta(t *testing.T) {
	...
	vdbEnv.config.MaxRetries = 1
	vdbEnv.config.MaxRetriesOnStartup = 1
	expectedErrMsg := fmt.Sprintf("http error calling mongodb: Get \"http://%s/testinithannelmetadata_/channel_metadata?attachments=true\": dial tcp %s: connect: connection refused",
		vdbEnv.config.Address, vdbEnv.config.Address)
...
func testRangeQueryWithInternalLimit(
	...
	require.Equal(t, int32(limit), db.mongoInstance.internalQueryLimit())

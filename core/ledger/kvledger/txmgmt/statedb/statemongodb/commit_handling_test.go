...
func TestGetRevision(t *testing.T) {
	...
	// load revision cache with mongoDB revision number.
...
func TestExecuteCommitter(t *testing.T) {
	...
	committers := []*committer{
		{
			db:             committerDB,
			batchUpdateMap: map[string]*batchableDocument{"key1": {MongoDoc: *mongoDocKey1}},
			namespace:      "ns",
		...
		{
			db:             committerDB,
			batchUpdateMap: map[string]*batchableDocument{"key2": {MongoDoc: *mongoDocKey2}},
			namespace:      "ns",
			cacheKVs:       make(cacheKVs),
			cacheEnabled:   true,
		},
	...
	err = db.executeCommitter(committers)
	require.EqualError(t, err, "error handling MongoDB request. Error:bad_request,  Status Code:400,  Reason:`docs` parameter must be an array.")
}
...
func TestCommitUpdates(t *testing.T) {
	...
	committerDB, err := db.getNamespaceDBHandle("ns")
	require.NoError(t, err)
	mongoDoc, err := keyValToMongoDoc(&keyValue{key: "key1", revision: "", VersionedValue: nsUpdates["key1"]})
	require.NoError(t, err)
	...
	var tests = []struct {
	...
	}{
		{
			...
			expectedErr: "error handling MongoDB request. Error:bad_request,  Status Code:400,  Reason:`docs` parameter must be an array.",
		},
		{
			committer: &committer{
				db:             committerDB,
				batchUpdateMap: map[string]*batchableDocument{"key1": {MongoDoc: *mongoDoc}},
				namespace:      "ns",
				cacheKVs:       make(cacheKVs),
				cacheEnabled:   true,
			},
		...
		},
		{
			...
			expectedErr: "error saving document ID: key1. Error: conflict,  Reason: Document update conflict.: error handling MongoDB request. Error:conflict,  Status Code:409,  Reason:Document update conflict.",
		},
	}
	...
	mongoDoc, err = keyValToMongoDoc(&keyValue{key: "key2", revision: "", VersionedValue: nsUpdates["key2"]})
	require.NoError(t, err)

	committer := &committer{
		db:             committerDB,
		batchUpdateMap: map[string]*batchableDocument{"key2": {MongoDoc: *mongoDoc}},
		namespace:      "ns",

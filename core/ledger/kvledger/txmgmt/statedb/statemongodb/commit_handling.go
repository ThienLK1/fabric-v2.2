...
func (vdb *VersionedDB) buildCommittersForNs(ns string, nsUpdates map[string]*statedb.VersionedValue) ([]*committer, error) {
	...
	maxBatchSize := db.mongoInstance.maxBatchUpdateSize()
	numCommitters := 1
	if maxBatchSize > 0 {
	...
	// for each committer, create a mongoDoc per key-value pair present in the update batch
	...
	for key, vv := range nsUpdates {
		...
		mongoDoc, err := keyValToMongoDoc(kv)
		if err != nil {
			return nil, err
		}
		committers[i].batchUpdateMap[key] = &batchableDocument{MongoDoc: *mongoDoc, Deleted: vv.Value == nil}
...
func (c *committer) commitUpdates() error {
	...
	for _, update := range c.batchUpdateMap {
		docs = append(docs, &update.MongoDoc)
	}
	...
	for _, resp := range responses {
		...
		if resp.Error != "" || resp.ID == "" {
			rev := string(resp.Rev)
			logger.Debugf("updateRevisionInCacheUpdate rev :%s", rev)
			c.updateRevisionInCacheUpdate(resp.ID, rev)
			continue
		}
		doc := c.batchUpdateMap[resp.ID]
		logger.Debugf("commitUpdates doc :%s", doc)
		var err error
		//Remove the "_rev" from the JSON before saving
		//this will allow the MongoDB retry logic to retry revisions without encountering
		//a mismatch between the "If-Match" and the "_rev" tag in the JSON
		if doc.MongoDoc.jsonValue != nil {
			err = removeJSONRevision(&doc.MongoDoc.jsonValue)
			if err != nil {
				return err
			}
		}
		...
		if doc.Deleted {
			logger.Warningf("MongoDB batch document delete encountered an problem. Retrying delete for document ID:%s", resp.ID)
			...
			logger.Warningf("MongoDB batch document update encountered an problem. Reason:%s, Retrying update for document ID:%s", resp.Reason, resp.ID)
			// Save the individual document to mongodb
			// Note that this will do retries as needed
			var revision string
			revision, err = c.db.saveDoc(resp.ID, "", &doc.MongoDoc)

...
func createMongoInstance(config *ledger.MongoDBConfig, metricsProvider metrics.Provider) (*mongoInstance, error) {
	logger.Debugf("constructing createMongoInstance %s",config)
	...
	if err != nil {
		return nil, errors.WithMessagef(
			err,
			"failed to parse MongoDB address '%s'",
			config.Address,
		)
	}

	mongoCfg := &MongoConnectionDef{finalURL.String(), config.Username, config.Password, "", config.CollectionName, config.MaxRetries,
		config.MaxRetriesOnStartup, config.RequestTimeout}

	//Create the MongoDB instance
	var client *mongo.Client

	if config.Username == "" || config.Password == "" {
		client, err = mongo.NewClient(options.Client().ApplyURI(mongoCfg.URL))
	} else {
		client, err = mongo.NewClient(options.Client().SetAuth(
			options.Credential{Username: mongoCfg.Username, Password: mongoCfg.Password, AuthSource: mongoCfg.AuthSource}).ApplyURI(mongoCfg.URL))
	}
...
func createMongoCollection(mongoInstance *mongoInstance, colName string) (*mongoDatabase, error) {
	...
	logger.Debugf("createMongoCollection dbName %s, collectionName %s", dbName, collectionName)
